        ...
        for side, (sh, el, wr) in [
            ...
            if in_motion:
                if self.phase == "DOWN" and elbow > WARN_DOWN:
                    warnings.append(f"{lbl} daha derin in ({elbow:.0f})")
                    lc[el] = YELLOW
                if self.phase == "UP" and elbow < WARN_UP:
                    warnings.append(f"{lbl} kolu tam duz ({elbow:.0f})")
                    lc[el] = YELLOW

        if in_motion and (visibility(landmarks, L_HI) > 0.5 and
                          visibility(landmarks, L_SH) > 0.5 and
                          visibility(landmarks, L_AN) > 0.5):
            p_sh = get_px(landmarks, L_SH, w, h)
            p_hi = get_px(landmarks, L_HI, w, h)
            p_an = get_px(landmarks, L_AN, w, h)
            body = calculate_angle(p_sh, p_hi, p_an)
            angles["Govde"] = round(body, 1)
            if body < 155:
                errors.append("Kalcayi yukseltme, govdeyi duz tut")
            elif body < 165:
                warnings.append("Govde hafif bukulu")
