...
FULL_CURL    = 55.0    # tam kivrilmis (UP fazi, kucuk aci)
FULL_EXTEND  = 150.0   # tam uzatilmis (DOWN fazi, buyuk aci)
WARN_CURL    = 80.0    # UP fazinda bu acinin ustu = yetersiz kivirma
WARN_EXTEND  = 120.0   # DOWN fazinda bu acinin alti = yetersiz uzatma
MAX_SHOULDER = 33.0    # dirsek one kayma esigi
MAX_TILT     = 0.09    # omuz esitlik esigi
MIN_BODY     = 164.0   # govde diklik esigi
    ...
    def analyze(self, landmarks, w, h):
        ...
        for side, (sh, el, wr, hi) in [
            ...
            if in_motion:
                if self.phase == "DOWN" and elbow < WARN_EXTEND:
                    ...
                    warnings.append(f"{lbl} daha fazla kivir ({elbow:.0f}>{WARN_CURL:.0f})")
                    lc[el] = YELLOW

                sh_angle = calculate_angle(p_hi, p_sh, p_el)
                if sh_angle > MAX_SHOULDER + 10:
                    errors.append(f"{lbl} dirsek one kaciyor")
                    lc[el] = RED
                elif sh_angle > MAX_SHOULDER:
                    warnings.append(f"{lbl} dirsek hafif one kaciyor")
        ...
        if in_motion:
            ...
            if tilt > MAX_TILT + 0.03:
                ...
                warnings.append("Omuz hafif egik")

            if visibility(landmarks, L_SH) > 0.5 and visibility(landmarks, L_HI) > 0.5:
                body_angle = calculate_angle(
                    get_px(landmarks, L_SH, w, h),
                    get_px(landmarks, L_HI, w, h),
                    [get_px(landmarks, L_HI, w, h)[0],
                     get_px(landmarks, L_HI, w, h)[1] + 100]
                )
                angles["Govde"] = round(body_angle, 1)
                if body_angle < MIN_BODY - 10:
                    errors.append("Arkaya yaslanma")
                elif body_angle < MIN_BODY:
