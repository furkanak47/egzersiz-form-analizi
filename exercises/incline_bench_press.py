...
class InclineBenchPress(BaseExercise):
    ...
    def __init__(self):
        super().__init__("Incline Bench Press")
    ...
    def analyze(self, landmarks, w, h):
        ...
        for side, (sh, el, wr) in [
            ...
            if in_motion:
                if self.phase == "UP" and elbow > WARN_DOWN:
                    warnings.append(f"{lbl} daha derin indir ({elbow:.0f})")
                    lc[el] = YELLOW
        ...
        if in_motion:
            sym = abs(landmarks[L_SH].y - landmarks[R_SH].y)
            if sym > 0.07:
