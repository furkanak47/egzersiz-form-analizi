...
HINGE_DOWN  = 75.0    # one egik (DOWN)
STAND_UP    = 165.0   # dik (UP)
WARN_HINGE  = 90.0
WARN_STAND  = 155.0
...
class Deadlift(BaseExercise):
    ...
    def analyze(self, landmarks, w, h):
        ...
        for side, (sh, hi, kn, an) in [
            ("L", (L_SH, L_HI, L_KN, L_AN)),
            ("R", (R_SH, R_HI, R_KN, R_AN)),
        ]:
        ...
        if in_motion and visibility(landmarks, L_SH) > 0.5 and visibility(landmarks, R_SH) > 0.5:
            back_tilt = abs(landmarks[L_SH].y - landmarks[R_SH].y)
            if back_tilt > 0.07:
